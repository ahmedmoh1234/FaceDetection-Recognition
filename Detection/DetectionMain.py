...
try:
    ...
    classifiersToBeUsedPath = os.path.join(
        dirname, 'classifiers5001-0-4-20.npy')
    classifiersToBeUsed = np.load(
        classifiersToBeUsedPath, allow_pickle=True)
...
print(
    f"Faces detected: {correctFacesCount} / {facesCount} , Accuracy: {correctFacesCount/facesCount * 100}%")
print(
    f"Non Faces detected: {correctNonFacesCount} / {nonFacesCount} , Accuracy: {correctNonFacesCount/nonFacesCount * 100}%")
...
def scaleFeatures(features, scale):
    ...
    try:
        finalFeaturesPath = os.path.join(
            dirname, 'scaledFeatures'+str(scale) + '.npy')
...
def predict(img2, classifiersToBeUsedFn) -> list:
    ...
    for features in scaledFeatures:
        ...
        print(
            f"Scale factor: {scaleFactor} and max scale factor: {maxScaleFactor}")
...
# imgPath = os.path.join(dirname, 'TestImg/Apple.jpg')
# img = io.imread(imgPath)
# detectedFace = detectFaces(img, classifiersToBeUsed, threshold)


def detector_main(input_image):
    detectedFace = detectFaces(input_image, classifiersToBeUsed, 30)
    return detectedFace != None
